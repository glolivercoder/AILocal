...
class OpenRouterClient:
    ...
    def __init__(self, api_key: str):
        if not api_key:
            # Tenta carregar do ambiente se não for fornecida diretamente
            api_key = os.getenv("OPENROUTER_API_KEY")
        
        if not api_key:
            logger.warning("Chave da API OpenRouter não encontrada.")
            raise ValueError("API Key para OpenRouter não fornecida ou encontrada no ambiente.")
...
class AiAgenteMCP:
    """Agente de IA com capacidades de MCP (Master Control Program)"""
    ...
    def __init__(self, config_file: str = "config/agent_config.json"):
        ...
        # Inicializar cliente OpenRouter com a chave do ambiente ou da config
        ...
        try:
            ...
            logger.error(f"Falha ao inicializar OpenRouterClient: {e}")
            self.openrouter_client = None

        self.tools = MCPTools()
        self.conversation_history = []
        self.current_mode = self.config.get("default_mode", "balanced")
        self.cache = {}
